	...
	$.fn.extend({
		colorPicker: function(config) {
			var renderCallback = function(colors, mode) {
					...
					$patch.css({
						'color': (colors.rgbaMixCustom.luminance > 0.22 ? '#222' : '#FFFFFF'), // Black...???
					...
					$(".st0").css({"fill": options._instance.toString()});
					$("#logo").css({"background-color": (colors.rgbaMixCustom.luminance > 0.22 ? '#535353' : '#c2c2c2')});
